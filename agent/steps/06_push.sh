...
Generated by agent/loop.sh"
  ...
  local VALIDATE_PROMPT FULL_DIFF VALIDATE_INPUT VALIDATE_OUTPUT VALIDATE_JSON VERDICT REASONING
...
\`\`\`"
  ...
  local validate_rc=0
  VALIDATE_OUTPUT=$(run_claude \
    --label "validate-diff-$TASK_ISSUE" \
    --timeout "$TIMEOUT_VERIFY_FIX" \
    --max-turns "$VALIDATE_DIFF_MAX_TURNS" \
    -- "$VALIDATE_INPUT") || validate_rc=$?
  ...
  if [[ "$validate_rc" -eq 124 ]]; then
    log "WARN: Diff validation timed out for #$TASK_ISSUE. Treating as pass."
    return 0
  fi
